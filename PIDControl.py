    ...
    while True:
        ...
        for i, room in enumerate(roomList):
            ...
            if int(time_now[0])>=0 and int(time_now[0])<=24:
                ...
                print("new={:.2f}-{:.2f}*({:.2f}-{:.2f})+{:.2f}*{:.2f}+{:.2f}*({:.2f}-2*{:.2f}+{:.2f})={:.2f}\n".format(var_open[i],Kp,error_now,error_last[i],Ki,error_now,Kd,error_now,error_last[i],error_llast[i],var_open[i] - Kp * (error_now - error_last[i]) - Ki * error_now - Kd * (error_now - 2 * error_last[i] + error_llast[i])))
                var_open[i] -= Kp * (error_now - error_last[i]) + Ki * error_now + Kd * (error_now - 2 * error_last[i] + error_llast[i])
                error_llast[i] = error_last[i]
